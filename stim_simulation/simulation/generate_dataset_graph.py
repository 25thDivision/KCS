...
사용법:
  python3 generate_dataset_graph.py
  python3 generate_dataset_graph.py -c color_code -d 3 -e X
  python3 generate_dataset_graph.py -c surface_code -n realistic/dp0_mf0.005_rf0.005_gd0.004 -d 3
"""
...
def send_discord_alert(code_type, noise, d, p, err_type, status, detail=""):
    log_to_file(f"Graph/{code_type}/{noise} | d={d}, p={p}, {err_type} | Generation {status.upper()} | Detail: {detail}")
    ...
    embed = {
        "title": f"{status_emoji} Graph Dataset Generation: {status.upper()}",
...
def send_completion_alert(total_files, elapsed_min=None):
    log_to_file("Graph dataset generation completed.")
    ...
    embed = {
        "title": "🎉 All Graph Datasets Generated Successfully!",
        ...
        "footer": {"text": f"STL Server | Workers: {NUM_WORKERS} | Generation Complete"}
    }
...
def generate_and_save(mapper, output_dir, code_type, d, p, err_type, total_samples, file_prefix, np_):
    ...
    for raw, phys in results:
        all_features.append(mapper.map_to_node_features(raw))
    ...
    np.savez_compressed(os.path.join(output_dir, file_name), features=full_features, labels=full_labels)
    print(f"       Saved: {file_name} (Shape: {full_features.shape})")

def main():
    start_time = time.time()
    file_count = 0
    print(f"=== Generating Graph Datasets ===")
    ...
    for code_type in CODE_TYPES:
        ...
        for noise in ACTIVE_NOISE:
            ...
            output_dir = PATHS.stim_data_dir(code_type, noise, "graph")
            print(f"\n{'#'*60}\n# {code_type}/{noise}\n# Params: {np_}\n{'#'*60}")
