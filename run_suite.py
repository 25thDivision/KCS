...
사용법:
  # 전체 파이프라인 (데이터 생성 → 학습)
  nohup python3 run_suite.py --all -c color -d 3 --gpu0 CNN GCN --gpu1 GraphMamba GraphTransformer > suite.log 2>&1 &

  # 데이터 생성만
  python3 run_suite.py --generate -c color surface -d 3

  # 학습 (GPU 2장 병렬)
  python3 run_suite.py --train -c color -d 3 --gpu0 CNN GCN GAT APPNP --gpu1 GCNII GNN GraphTransformer GraphMamba

  # 학습 (GPU 1장)
  python3 run_suite.py --train -c surface -d 3 -g 0 -m GraphMamba GraphTransformer
  ...
  # 데이터 생성 + 학습 + 실험 한번에
  nohup python3 run_suite.py --all --experiment ionq -c color -d 3 -g 0 -m GraphMamba GraphTransformer > suite.log 2>&1 &
...
# cmd 실행
# ==============================================================================
def run_cmd(cmd, cwd=None, log=None, async_run=False):
    cmd_str = " ".join(cmd)
    print(f"  {'🚀 [Parallel]' if async_run else '▶️'} {cmd_str}")
    ...
    if log:
        ...
        if async_run:
            f = open(log_path, "w")
            return subprocess.Popen(cmd, cwd=cwd, stdout=f, stderr=subprocess.STDOUT), f
        else:
...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python3 run_suite.py --generate -c color -d 3
  python3 run_suite.py --train -g 0 -m GraphMamba -c color -d 3
  python3 run_suite.py --train --gpu0 CNN GCN --gpu1 GraphMamba GraphTransformer -c surface -d 3
  python3 run_suite.py --experiment ionq -m GraphMamba GraphTransformer
  python3 run_suite.py --experiment ibm -m CNN GNN
  python3 run_suite.py --all --experiment ionq -c color -d 3 -m GraphMamba GraphTransformer
  python3 run_suite.py --status
        """
    )
    ...
    common.add_argument("--id", type=str, default=None,
                        help="Pipeline ID for log folder. If not specified, uses auto-generated PIPELINE_ID.")
    ...
    experiment.add_argument("-b", "--backend", type=str, default=None, help="IBM 실험 백엔드 선택 (예: ibm_boston)")
    experiment.add_argument("-i", "--instance", type=str, default=None,
                            help="Qiskit Runtime Service instance (예: Yonsei_internal)")
...
def do_generate(args):
    ...
    if args.cpu:
        extra += ["--cpu"] + [args.cpu]
        log.append(f"cpu-{args.cpu}")

    print(f"\n  --- Graph datasets ---")
    run_cmd(base_cmd_graph + extra, cwd=STIM_SIM_DIR, log=f"graph_generation_{'_'.join(log)}.log")

    print(f"\n  --- Image datasets ---")
    run_cmd(base_cmd_image + extra, cwd=STIM_SIM_DIR, log=f"image_generation_{'_'.join(log)}.log")
...
def do_train(args):
    ...
    if args.gpu0 or args.gpu1:
        ...
        if args.gpu0:
            ...
            log0 = [f"gpu0"]
            log0.append(f"{'-'.join(args.gpu0)}")
            if args.code:
                cmd0 += ["-c"] + args.code
                log0.append(f"{'-'.join(args.code)}")
            if args.distance:
        ...
        if args.gpu1:
            ...
            log1 = [f"gpu1"]
            log1.append(f"{'-'.join(args.gpu1)}")
            if args.code:
                cmd1 += ["-c"] + args.code
                log1.append(f"{'-'.join(args.code)}")
            if args.distance:
        ...
        if args.gpu is not None:
            cmd += ["-g"] + [str(args.gpu)]
            log.append(f"gpu{args.gpu}")
        if args.models:
            cmd += ["-m"] + args.models
            log.append(f"{'-'.join(args.models)}")
        if args.code:
            cmd += ["-c"] + args.code
            log.append(f"{'-'.join(args.code)}")
        if args.distance:
...
def do_experiment(args):
    ...
    for platform in args.experiment:
        ...
        if args.models:
            cmd += ["-m"] + args.models
            log.append(f"{'-'.join(args.models)}")
        if platform == "ibm" and args.backend:
            cmd += ["-b"] + [args.backend]
...
def do_status(args):
    ...
    #         mtime = datetime.fromPIPELINE_ID(os.path.getmtime(log_path)).strftime("%m/%d %H:%M")
...
def main():
    ...
    global PIPELINE_ID
    pipeline_id = args.id if args.id else PIPELINE_ID
    PIPELINE_ID = pipeline_id
    os.environ["KCS_PIPELINE_ID"] = pipeline_id
