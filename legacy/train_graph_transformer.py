...
# [학습 하이퍼파라미터]
BATCH_SIZE = 1024
LEARNING_RATE = 1e-4  # 안정적인 학습을 위해 1e-4 권장
MAX_EPOCHS = 20
PATIENCE = 3
OPTIMIZER_NAME = "Adam"
...
def train_one_epoch(model, loader, optimizer, criterion, device, edge_index): 
    model.train()
    total_loss = 0.0
    # tqdm은 에포크마다 출력하면 너무 많을 수 있으니 leave=False로 설정
...
# [수정] 매 Epoch마다 로그를 저장하는 함수 (파라미터 추가)
    ...
    for distance in DISTANCES:
        for error_rate in ERROR_RATES:
            for error_type in ERROR_TYPES:
                ...
                print(f"\n{'='*60}")
                print(f"=== Training Start: d={distance}, p={error_rate}, Type={error_type} ===")
                print(f"{'='*60}")
